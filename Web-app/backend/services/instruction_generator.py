...
class InstructionGenerator:
    ...
    def generate_requests(
        self, ai_result: dict, template: dict, original_doc: dict
    ) -> list[dict]:
        """
        ...
        if columns > 1 and "columns_break_index" in formatted_content:
            ...
            if break_idx > 1 and break_idx < len(formatted_content["text"]):
                ...
                col_spacing = template.get("columnSpacing", 24)
                col_props = [
                    {"paddingEnd": {"magnitude": col_spacing, "unit": "PT"}}
                ...
                # Ensure the first section is strictly 1-column
                requests.append(
                    {
                        "updateSectionStyle": {
                            "range": {"startIndex": 1, "endIndex": break_idx},
                            "sectionStyle": {
                                "columnProperties": [
                                    {"paddingEnd": {"magnitude": 0, "unit": "PT"}}
                                ]
                ...
                requests.append(
                    {
                        "updateSectionStyle": {
                            ...
                            "sectionStyle": {"columnProperties": col_props},
                            "fields": "columnProperties",
                        }
                    }
                )
    ...
    def _build_formatted_content(self, ai_result: dict, template: dict) -> dict:
        ...
        has_structure = "structure" in template and template.get("structure", {}).get(
    ...
    def _build_custom_template_content(self, ai_result: dict, template: dict) -> dict:
        ...
        # Get styles
        title_style = template_styles.get(
            "TITLE", {"fontSize": 24, "bold": True, "alignment": "CENTER"}
        )
        heading1_style = template_styles.get(
            "HEADING_1", {"fontSize": 16, "bold": True, "alignment": "START"}
        )
        normal_style = template_styles.get(
            "NORMAL_TEXT", {"fontSize": 12, "alignment": "START", "lineSpacing": 1.5}
        )
        ...
        # Main Title
        main_title = cover_page.get(
            "main_title", ai_result.get("title", "Untitled Document")
        )
        if main_title:
    ...
    def _build_default_template_content(self, ai_result: dict, template: dict) -> dict:
        ...
        # ---------- SAFE DATA ----------
        title = ai_result.get("title") or "A Study on Emerging Technologies"
        authors = ai_result.get("authors", "")
        abstract = ai_result.get("abstract", "")
        keywords = ai_result.get("keywords", "")
        sections = ai_result.get("sections", [])
        conclusion = ai_result.get("conclusion", "").strip()

        # Avoid duplicate conclusion
        if sections and conclusion:
            last_section_content = sections[-1].get("content", "")
            if conclusion in last_section_content:
                conclusion = ""

        # ---------- FIX BAD TITLE ----------
        if title.lower() in ["research paper", "untitled document"]:
            title = "A Study on Emerging Technologies and Their Impact on Society"
        ...
        # ---------- ABSTRACT ----------
        if abstract and abstract.strip().lower() not in [
            "",
            "abstract not provided",
            "not provided",
            "abstract not provided.",
        ]:
            if not abstract.startswith("Abstract—"):
        ...
        for i in range(len(section_names)):
            content = sections[i].get("content", "") if i < len(sections) else ""
            if not content.strip():
                content = "This section discusses key aspects related to the topic and provides relevant insights."
            else:
                # Split using regex but prevent splitting on common abbreviations and acronyms
            ...
            heading = f"{roman[i]}. {section_names[i]}"
        ...
        # ---------- CONCLUSION ----------
        if conclusion:
            heading = f"{roman[len(section_names)]}. CONCLUSION\n"
    ...
    def _create_page_setup_requests(self, template: dict) -> list[dict]:
        ...
        if margins:
            ...
            if page_size:
                doc_style["pageSize"] = {
                    "width": {
                        "magnitude": page_size.get("width", 595.276),
                        "unit": "PT",
                    },
                    "height": {
                        "magnitude": page_size.get("height", 841.89),
                        "unit": "PT",
                    },
                }
