...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function updateCtaForBrowser() {
    ...
    if (browser !== "chrome") {
      ...
      allCtaButtons.forEach((button) => {
        ...
        if (textSpan) {
          textSpan.textContent = "Get FormatX (Chrome Required)";
        }
      });
  ...
  /*
    function showBrowserMessage(browser) {
        ...
        `;
        message.innerHTML = `
            <strong>Note:</strong> FormatX is a Chrome extension. 
...
function showExitIntent() {
    ...
    modal.innerHTML = `
        ...
        <p style="margin-bottom: 1.5rem; color: #7a7695;">
            Install FormatX now and format your first document in seconds.
        </p>
...
function abTestCta() {
    ...
    if (variant === 'A') {
        ...
        textSpan.textContent = 'Get FormatX Free';
    }
    
    // Track which variant user sees
    localStorage.setItem('formatx_cta_variant', variant);
...
function showInstallCount() {
    ...
    //         `;
    //         badge.innerHTML = `
    //             <strong style="color: #7b6cff;">${formatted}+</strong> users formatting documents with FormatX
    //         `;
